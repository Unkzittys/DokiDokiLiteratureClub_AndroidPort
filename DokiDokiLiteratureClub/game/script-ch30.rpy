...
init python:
    ...
    def slow_nodismiss(event, interact=True, **kwargs):
        if not persistent.monika_kill:
            if renpy.android:
                try:
                    file(os.environ['ANDROID_PUBLIC'] + "/characters/monika.chr")
                except:
...
label ch30_main2:
    ...
    if persistent.steam:
        ...
        m "Well, you're playing on Android, so it was actually a bit more difficult..."
        m "To go into the game directory, you have to look in your files for the folders 'Android/data'."
        $ android_package1 = os.environ['ANDROID_PUBLIC']
        $ android_package2 = android_package1.replace('/storage/emulated/0/Android/data/', '')
        $ android_package3 = android_package2.replace('/files', '')
        m "Then, all the files I needed were in the '[android_package3]' folder, and I could just do whatever I wanted..."
...
label ch30_endb:
    ...
    $ consolehistory = []
    if renpy.android:
        call updateconsole ("file(\"characters/monika.chr\")", "monika.chr does not exist.")
    else:
...
label ch30_reload_2:
    ...
    if renpy.android:
        $ android_basedir = os.environ['ANDROID_PUBLIC']
        m "I'm pretty sure you can find it in the folder called [android_basedir]/characters."
    else:
    ...
    if persistent.steam:
        ...
        m "Well, you're playing on Android, so you can just go into your files app and look for something that suggests bring you to the 'Internal Storage'."
...
label ch30_reload_4:
    ...
    if renpy.android:
        $ android_basedir = os.environ['ANDROID_PUBLIC']
        m "It's in [android_basedir]/characters."
    else:
